...
| Слой | Что делает |
|------|------------|
| **WireGuard** | VPN-туннель на устройстве; конфиг запрашивается у сервера командой `GETCONF` по DTLS |
| **DTLS** | Шифрованный канал клиент ↔ VPS через TURN |
| **TURN (VK)** | UDP relay; трафик идёт через серверы звонков VK |
| **WRAP** | Ключ **не в APK**: HKDF-SHA256 от пароля подключения (`WDTT-WRAP-v1` / `rtp-obfs/chacha20poly1305`) |
| **RTP AEAD** | 12-байтный RTP-заголовок (SSRC, seq, timestamp = nonce) + ChaCha20-Poly1305; payload type **111 (OPUS)**; случайный padding |

Один пароль подключения = один WRAP-ключ. Сервер держит набор ключей в памяти и обновляет их при смене паролей без рестарта (`SIGHUP`).
