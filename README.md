...
Совместим с сервером и протоколом WDTT (оригинал: amurcanov/proxy-turn-vk-android). Сервер wdtt-server, ссылки wdtt://, WRAP/RTP и анонимный VK-auth работают так же — можно поднять VPS оригинальным деплоем и подключиться к OBhoD_BLOK.

OBhoD_BLOK и базовый WDTT — в чём разница
Клиент OBhoD_BLOK и базовый WDTT используют один и тот же протокол. Серверные части полностью совместимы: wdtt-server с деплоя оригинала или OBhoD_BLOK, пароли, wdtt://, WRAP/RTP, Telegram-бот — всё работает взаимозаменяемо. Меняется в основном Android-клиент (UI и удобство).

Есть в OBhoD_BLOK, чего нет в базовом WDTT:

Функция	OBhoD_BLOK Базовый WDTT
...
OBhoD_BLOK  использует инфраструктуру VK-звонков как транспорт:
...
Расширенная ссылка OBhoD_BLOK (qwdtt://)
...
Сервер: вкладка Деплой или уже настроенный VPS с wdtt-server.
VK: групповой звонок → ссылка vk.com/call/join/… или только хеш после /join/.
Импорт: вставьте wdtt://… / qwdtt://…, QR или заполните профиль вручную.
Подключить → разрешить VPN.
...
Происхождение
Форк proxy-turn-vk-android (автор amurcanov). 
